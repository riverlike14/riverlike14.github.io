...
We are going to learn how to wrap `<button />` HTML element.
...
- If props(`primary`, `secondary`, ...) are not passed down, props will be `undefined`.
...
- Therefore, we have to figure out what styling rules we need to get a `primary` looking button and build it from scratch.
...
- Must designate style on each class name.
...
- We have to edit `index.css` file, which eventually solves this problem.
- ```css
  /* index.css */
  @tailwind base;
  @tailwind components;
...
## Special Props Name: `className`

User can pass such as `className="mb-5"` props to the `<Button />` componne.
- For the moment, `<Button className="mb-5" primary />` will not work.
